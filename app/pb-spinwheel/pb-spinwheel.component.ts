...
export class PbSpinwheelComponent implements OnInit {
  ...
  loadSpinWheelRules() {
    ...
    Playbasis.engineApi.listRules({action: this.envTargetAction})
      .then((result: any) => {
        ...
        // find possible rules
        let rules = selfObj.findRulesWithTargetTagAndHaveCustomUrlValuesThatPassedUrlValuesCriteria(result.response, selfObj.envCustomParamUrlValues);
  ...
  getRotationAngleForTargetSectionIndex(index: number): number {
    ...
    let retAngle = Math.floor(Math.random() * (maxAngle - minAngle)) + minAngle;
  ...
  markTargetSectionIndex() {
    ...
    let type = -1;
    let rewardType = this._gotRewardItem.reward_type;
    let rewardValToCheckAgainst;
    let isGoodsGroup = false;
  ...
  findRulesWithTargetTagAndHaveCustomUrlValuesThatPassedUrlValuesCriteria(rulesResponse: any, customUrlValues: Array<string>): Array<any> {
    ...
    // with spin wheel
    for (let i = 0; i < rulesResponse.length; i++) {
      let r = rulesResponse[i];

      // return -1 if not found
      if (r.tags.search(this.envTargetTag) !== -1) {
        // only if this rule has url param set
        if (r.jigsaw_set != null) {
          for (let j = 0; j < r.jigsaw_set.length; j++) {
            let jigsaw = r.jigsaw_set[j];
            ...
            if (jigsaw.name === 'customParameter' &&
              ...
              for (let k = 0; k < customUrlValues.length; k++) {
  ...
  spinWheel(targetDegree: number) {
    ...
    let newDegree = this._degree * (this._spinChanceSuccessCount + 1);
    let totalDegree = newDegree + targetDegree;
